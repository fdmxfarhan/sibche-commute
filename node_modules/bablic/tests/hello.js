...
describe('Hello', function () {
    ...
    it('should return translated HTML', function (done) {
        request({
            ...
            chai_1.expect(e).null;
            if (e)
                return done();
            chai_1.expect(response.statusCode).equal(200);
            chai_1.expect(body).include('CARGANDO...');
            chai_1.expect(body).include('hecho por');
            done();
        });
    });
    it("should not translate without user agent", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.statusCode).equal(200);
            chai_1.expect(body).include('LOADING...');
            chai_1.expect(body).include('made by');
            done();
        });
    });
    it("should return header with spanish", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.headers['x-locale']).equal('es');
            done();
        });
    });
    it("should return header with english", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.headers['x-locale']).equal('en');
            done();
        });
    });
    it("should write in snippet", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(body).include('cdn2.bablic.com/js/bablic');
            done();
        });
    });
    it("should redirect old URLs", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.statusCode).equal(301);
            chai_1.expect(response.headers['location']).equal('/sobre?locale=es');
            done();
        });
    });
    it("should should return 404 on spanish page without locale", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.statusCode).equal(404);
            done();
        });
    });
    it("should should return 200 on spanish page with locale", done => {
        request({
            ...
            if (e)
                return done();
            chai_1.expect(response.statusCode).equal(200);
            chai_1.expect(body).include('About us');
            done();
        });
    });
});
