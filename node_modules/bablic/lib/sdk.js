...
class BablicSDK {
    ...
    getLocale(req) {
        ...
        return common_1.getLocaleByURL(url_parser.parse(getCurrentUrl(req)), localeDetection, customUrls, detectLocaleFromCookie(req, this.meta), defaultLocale, auto ? detectLocaleFromHeader(req) : "", false, this.options.locale, this.options.subDirBase, this.options.folders, localeKeys);
    }
    loadSiteMeta(cbk) {
    ...
    altTags(url, locale) {
        ...
        let tags = _(locales)
            ...
            .without(locale)
            .map((l) => `<link rel="alternate" href="${this.getLink(l, url)}" hreflang="${l == this.meta.original ? "x-default" : l}">`)
            .valueOf();
        return tags.join("");
    }
    purgeCache() {
