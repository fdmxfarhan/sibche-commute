...
router.get('/admin-general-report', ensureAuthenticated, (req, res, next) => {
    if(req.user.role == 'admin')
    {
        ...
        for(var i=0; i<12; i++){
            report.push({});
            report[i].wb = new excel.Workbook({dateFormat: 'm/d/yy hh:mm:ss'});
            report[i].style = report[i].wb.createStyle({
                font: {
                    color: '#111111',
                    size: 12
                },
            ...
            report[i].ws = report[i].wb.addWorksheet('گزارش کلی');
            report[i].ws.cell(1,1).string(`شماره پرسنل`).style(report[i].style);
            report[i].ws.cell(1,2).string(`net`).style(report[i].style);
            report[i].ws.cell(1,3).string(`total`).style(report[i].style);
            report[i].ws.cell(1,4).string(`تعداد حضور`).style(report[i].style);
            report[i].ws.cell(1,5).string(`تعداد غیبت`).style(report[i].style);
            report[i].ws.cell(1,6).string(`اضافه کاری`).style(report[i].style);
            report[i].ws.cell(1,7).string(`تاخیر`).style(report[i].style);
            report[i].ws.cell(1,8).string(`غیبت ساعتی`).style(report[i].style);
        }
        Commute.find({}, (err, allCommutes) => {
            data = [];
            for(var userNum = 0; userNum < users.length; userNum++){
                var commutes = allCommutes.filter(element => element.personelID == users[userNum]);
                commutes.sort(sortAlgorithm);
                for(var monthNum=1; monthNum<=12; monthNum++){
                    monthData = [];
                    cnt = 2;
                    ...
                    report[monthNum-1].ws.cell(userNum+2,1).string(`${users[userNum]}`).style(report[monthNum-1].style);
                    report[monthNum-1].ws.cell(userNum+2,2).string(timeToString(monthSum)).style(report[monthNum-1].style);
                    report[monthNum-1].ws.cell(userNum+2,3).string(timeToString(monthTotal)).style(report[monthNum-1].style);
                    report[monthNum-1].ws.cell(userNum+2,4).string(`${presence}`).style(report[monthNum-1].style);
            ...
            fs.mkdirSync(path.join(__dirname, "/../public/output"));
            for(var i=0; i<12; i++){
                report[i].wb.write(`./public/output/گزارش_کلی_${dateConvert.get_persian_month(i+1)}.xlsx`);
                data.push({});
                data[i].out = `/output/گزارش_کلی_${dateConvert.get_persian_month(i+1)}.xlsx`;
                data[i].name = `${dateConvert.get_persian_month(i+1)}`;
            }
            res.render('./dashboard/admin-general-report', {
                user: req.user,
                report: data,
            });
        });
